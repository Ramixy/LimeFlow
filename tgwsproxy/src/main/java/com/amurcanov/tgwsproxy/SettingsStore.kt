...
class SettingsStore(private val context: Context) {
    ...
    val themePalette: Flow<String> = context.dataStore.data.map { it[Keys.THEME_PALETTE] ?: "limeflow" }
    ...
    suspend fun saveThemeMode(mode: String) {
        appearancePreferences.edit().putString("app_theme", mode).apply()
        context.dataStore.edit { it[Keys.THEME_MODE] = mode }
    }

    suspend fun saveDynamicColor(enabled: Boolean) {
        appearancePreferences.edit().putBoolean("app_dynamic_colors", enabled).apply()
        context.dataStore.edit { it[Keys.IS_DYNAMIC_COLOR] = enabled }
    }

    suspend fun saveThemePalette(palette: String) {
        appearancePreferences.edit().putString("app_palette", palette).apply()
    ...
    suspend fun migrateLegacyDefaults() {
        context.dataStore.edit {
            if (it[Keys.UNIFIED_APPEARANCE_MIGRATED] != true) {
                it[Keys.THEME_MODE] = "system"
                it[Keys.IS_DYNAMIC_COLOR] = true
                it[Keys.UNIFIED_APPEARANCE_MIGRATED] = true
    ...
    suspend fun exportSettings(): JSONObject {
        ...
        context.dataStore.data.first().asMap().forEach { (key, value) ->
            ...
            output.put(key.name, item)
        }
        return output
    }

    suspend fun importSettings(input: JSONObject) {
        context.dataStore.edit { preferences ->
            preferences.clear()
            input.keys().forEach { name ->
                val item = input.optJSONObject(name) ?: return@forEach
                when (item.optString("type")) {
                    "boolean" ->
                        preferences[booleanPreferencesKey(name)] = item.getBoolean("value")
                    "int" ->
                        preferences[intPreferencesKey(name)] = item.getInt("value")
                    "long" ->
                        preferences[longPreferencesKey(name)] = item.getLong("value")
                    "float" ->
                        preferences[floatPreferencesKey(name)] = item.getDouble("value").toFloat()
                    "string" ->
                        preferences[stringPreferencesKey(name)] = item.getString("value")
                    "string_set" -> {
                        val values = item.getJSONArray("value")
                        preferences[stringSetPreferencesKey(name)] = buildSet {
